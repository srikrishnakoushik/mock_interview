...
const Loading3D = () => (
  // Fix: Wrap the HTML content in <Html> component
  <Html center>
...
const JobDescriptionInput = ({ onQuestionsGenerated }) => {
  ...
  useEffect(() => {
    // Cleanup speak function if component unmounts or state changes
    return () => {
      if (speaking) {
        cancel();
      }
...
const QuestionGenerationView = ({ questionsData, onStartInterview }) => {
  ...
  }, [questionsData, speak, speaking, cancel]);
...
// Refactored Recorder Component with Audio-Only Recording and delayed evaluation
const Recorder = ({ questionsData, onInterviewComplete }) => {
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const [isRecording, setIsRecording] = useState(false);
  const [recordingTime, setRecordingTime] = useState(0);
  const [answers, setAnswers] = useState([]); // Stores { question, transcript, recordingTime, audioBlob, session_id }
  const [isTranscribing, setIsTranscribing] = useState(false);
  const [audioStream, setAudioStream] = useState(null); // Changed from videoStream to audioStream for clarity
  ...
  // --- FIX: Define processRecording BEFORE startRecording and stopRecording ---
  // Refactored to only transcribe and store locally
  const processRecording = useCallback(async (audioBlob) => { // Renamed param to audioBlob for clarity
    console.log("Recorder: Processing recorded audio blob...");
    try {
      const formData = new FormData();
      formData.append('file', audioBlob, 'recording.webm'); // Still use .webm extension for consistency
      formData.append('session_id', questionsData.session_id);
      formData.append('question_index', currentQuestionIndex.toString());

      console.log("Recorder: Sending for transcription to backend...");
      ...
      console.log("Recorder: Received transcript:", transcript);
      ...
      const newAnswer = {
        ...
        audioBlob: audioBlob, // Store audio blob here
        session_id: questionsData.session_id
      };

      setAnswers(prev => {
        const updatedAnswers = [...prev, newAnswer];
        console.log("Recorder: Answers updated. Current count:", updatedAnswers.length);
        // Determine next action after state update
        const nextQuestionIndex = currentQuestionIndex + 1;
        if (nextQuestionIndex < questionsData.questions.length) {
          console.log("Recorder: Moving to next question.");
          // Use a timeout to allow speech to finish before next question is spoken
          setTimeout(() => {
            setCurrentQuestionIndex(nextQuestionIndex);
            // Optional: auto-start recording for next question
            // startRecording(); // <--- This would now be accessible after reordering
          }, 2000); // 2-second delay
        } else {
          console.log("Recorder: All questions answered. Triggering interview complete.");
          speak({ text: "You've completed all questions. Preparing your feedback report now." });
          // Pass all collected answers to the parent to switch to FeedbackReport view
          onInterviewComplete(updatedAnswers); // Pass the fully updated answers array
        }
        return updatedAnswers; // Return the new state
      });

    } catch (error) {
      console.error("Recorder: Error transcribing recording or saving answer:", error);
      speak({ text: "There was an error transcribing your answer. Please try again." });
      alert("Error transcribing your answer. Please try again. Check console for details.");
    } finally {
      setIsTranscribing(false);
      setRecordingTime(0);
      console.log("Recorder: Transcription process finished.");
    }
  }, [currentQuestionIndex, questionsData, recordingTime, onInterviewComplete, speak, answers]);


  // Define startRecording AFTER processRecording
  const startRecording = useCallback(async () => {
    console.log("Recorder: Attempting to start recording...");
    try {
      if (!audioStream) { // Use audioStream
        alert("Microphone not available. Please allow microphone access.");
        console.error("Recorder: Cannot start recording, audioStream is null.");
        return;
      }
      if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
          console.warn("Recorder: Already recording, ignoring start request.");
          return;
      }

      chunksRef.current = [];
      mediaRecorderRef.current = new MediaRecorder(audioStream, { // Use audioStream
        mimeType: 'audio/webm' // IMPORTANT: Changed to audio/webm
      });

      mediaRecorderRef.current.ondataavailable = (event) => {
        if (event.data.size > 0) {
          chunksRef.current.push(event.data);
          console.log("Recorder: Data available, chunk size:", event.data.size);
        }
      };

      mediaRecorderRef.current.onstop = async () => {
        console.log("Recorder: MediaRecorder stopped. Processing blob...");
        const blob = new Blob(chunksRef.current, { type: mediaRecorderRef.current.mimeType });
        console.log("Recorder: Blob created, size:", blob.size, "type:", blob.type);
        await processRecording(blob);
      };

      mediaRecorderRef.current.start(); // Start without time slice
      setIsRecording(true);
      setRecordingTime(0);
      ...
      speak({ text: "Could not start recording. Please check microphone permissions." });
      alert("Could not start recording. Please check microphone permissions.");
    }
  }, [audioStream, speak, processRecording]); // processRecording is now in scope


  const stopRecording = useCallback(() => {
    console.log("Recorder: Attempting to stop recording...");
    if (mediaRecorderRef.current && mediaRecorderRef.current.state === 'recording') {
      mediaRecorderRef.current.stop();
      setIsRecording(false);
      clearInterval(recordingIntervalRef.current);
      setIsTranscribing(true);
      console.log("Recorder: Recording stopped. Transcribing initiated.");
      speak({ text: "Recording stopped. Transcribing your answer now." });
    } else {
      console.warn("Recorder: MediaRecorder not active, cannot stop.");
    }
  }, [speak]);


  // Initialize microphone (audio-only) and speak first question
  useEffect(() => {
    const initMicrophone = async () => { // Renamed for clarity
      console.log("Recorder: Attempting to initialize microphone...");
      try {
        ...
        console.log("Recorder: Microphone initialized successfully.");
      } catch (error) {
        console.error("Recorder: Error accessing microphone:", error);
    ...
    if (questionsData && questionsData.questions[currentQuestionIndex] && !speaking) {
      console.log(`Recorder: Speaking question ${currentQuestionIndex + 1}`);
      speak({ text: `Question ${currentQuestionIndex + 1}: ${questionsData.questions[currentQuestionIndex]}` });
    }

    return () => {
      console.log("Recorder: Cleaning up effects...");
      if (audioStream) {
        console.log("Recorder: Stopping audio stream tracks.");
        audioStream.getTracks().forEach(track => track.stop());
      }
      if (speaking) {
        console.log("Recorder: Cancelling speech.");
        cancel();
      }
      if (recordingIntervalRef.current) {
          console.log("Recorder: Clearing recording interval.");
          clearInterval(recordingIntervalRef.current);
      }
    };
  }, [questionsData, currentQuestionIndex, speak, audioStream, speaking, cancel]); // Dependencies
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <Canvas camera={{ position: [0, 0, 5], fov: 50 }}>
                  ...
                  <Suspense fallback={<Loading3D />}>
                    <InterviewerAvatar
                      ...
                      isListening={isRecording && !speaking && !isTranscribing}
                      // Avatar is speaking when AI is speaking or transcribing
                      isSpeaking={speaking || isTranscribing}
                    />
                    <OrbitControls
                      enableZoom={false}
                      enablePan={false}
          ...
          {/* Right side - Audio Recording Controls and Status */}
          <div className="space-y-6">
            {/* Audio Recording Status */}
            <div className="bg-white rounded-2xl shadow-xl p-6 text-center">
                <h3 className="text-xl font-bold text-gray-800 mb-4">Your Audio Recording</h3>
                <p className="text-gray-600">Microphone ready for recording.</p>
                {/* No <video> tag here, as we're doing audio-only */}
            </div>
              ...
              <div className="text-center">
                ...
                {isTranscribing && (
                  <div className="flex items-center justify-center">
                    ...
                    <span className="text-lg text-purple-600 font-semibold">Transcribing your answer...</span>
                  </div>
                )}
              </div>
            </div>
        ...
        {answers.length > 0 && (
          <div className="bg-white rounded-2xl shadow-xl p-6 mt-8">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Previous Answers (Not yet evaluated)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {answers.map((answer, index) => (
                <div key={index} className="bg-gray-50 p-4 rounded-lg border-l-4 border-blue-500">
                  <div className="flex justify-between items-start mb-2">
                    <span className="font-semibold text-gray-800">Q{answer.questionIndex + 1}</span>
                    <span className="text-sm text-gray-600">{formatTime(answer.recordingTime)}</span>
                  </div>
...
// Evaluation Logic in FeedbackReport
const FeedbackReport = ({ answers: rawAnswers, onRestart }) => {
  const [evaluatedAnswers, setEvaluatedAnswers] = useState([]);
  const [isEvaluatingAll, setIsEvaluatingAll] = useState(true);
  const { speak, cancel, speaking } = useSpeechSynthesis();

  // Evaluate all answers when the component mounts
  useEffect(() => {
    const evaluateAll = async () => {
      console.log("FeedbackReport: Starting evaluation of all answers.");
      setIsEvaluatingAll(true);
      speak({ text: "Evaluating all your answers now. Please wait a moment." });
      const results = [];
      for (const answer of rawAnswers) {
        try {
          console.log(`FeedbackReport: Evaluating question ${answer.questionIndex + 1}. Transcript: "${answer.transcript}"`);
          const evaluationResponse = await axios.post(`${API}/evaluate`, {
            transcript: answer.transcript,
            question: answer.question,
            session_id: answer.session_id // Send session_id for job_description lookup
          });
          results.push({ ...answer, evaluation: evaluationResponse.data });
          console.log(`FeedbackReport: Evaluated question ${answer.questionIndex + 1}. Score: ${evaluationResponse.data.score}`);
          speak({ text: `Evaluated question ${answer.questionIndex + 1}.` });
        } catch (error) {
          console.error(`FeedbackReport: Error evaluating question ${answer.questionIndex + 1}:`, error);
          results.push({
            ...answer,
            evaluation: {
              score: 0,
              ...
              suggestions: ["Check backend logs for details on the evaluation API call."]
            }
          });
          speak({ text: `Failed to evaluate question ${answer.questionIndex + 1}.` });
        }
      }
      setEvaluatedAnswers(results);
      setIsEvaluatingAll(false);
      const finalAverage = calculateAverageScore(results);
      console.log("FeedbackReport: All evaluations complete. Final average score:", finalAverage);
      speak({ text: `All evaluations complete! Your average score was ${finalAverage.toFixed(1)} out of 10. Review your report.` });
    };

    // Only run evaluation if there are raw answers and it hasn't started yet
    if (rawAnswers.length > 0 && isEvaluatingAll) {
        evaluateAll();
    } else if (rawAnswers.length === 0 && !isEvaluatingAll) {
        // If somehow we land here with no answers, but evaluation is not active, reset
        setEvaluatedAnswers([]);
        setIsEvaluatingAll(false);
    }
  ...
  }, [rawAnswers, speak, cancel, isEvaluatingAll]);
  ...
  const averageScore = calculateAverageScore(evaluatedAnswers);
  const totalTime = rawAnswers.reduce((sum, answer) => sum + answer.recordingTime, 0);
        ...
        {isEvaluatingAll ? (
          <div className="flex items-center justify-center h-64 w-full">
            <div className="text-center">
              ...
              <p className="text-gray-600">Evaluating answers...</p>
              {speaking && (
                  <div className="mt-3 flex items-center justify-center text-blue-600">
                    <div className="animate-pulse w-2 h-2 bg-blue-600 rounded-full mr-1"></div>
                    <div className="animate-pulse w-2 h-2 bg-blue-600 rounded-full mr-1 delay-100"></div>
          ...
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="bg-white rounded-2xl shadow-xl p-6 text-center">
                <div className="text-3xl font-bold text-green-600 mb-2">{evaluatedAnswers.length}</div>
                    ...
                    <div className="space-y-4">
                      ...
                      {answer.audioBlob && (
                          <div className="bg-gray-100 p-4 rounded-xl">
                              <p className="text-gray-700 font-medium mb-2">Your Recording:</p>
                              {/* Use <audio> tag for audio-only blobs */}
                              <audio controls src={URL.createObjectURL(answer.audioBlob)} className="w-full rounded-lg"></audio>
                          </div>
                      )}
                    </div>
...
const App = () => {
  ...
  const handleInterviewComplete = useCallback((answers) => {
